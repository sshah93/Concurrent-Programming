...
char *buffer;
sem_t stringGoneSemaphore;
sem_t stringIsInSemaphore;
...
/* initialize shared buffer */
  if ((rc = sem_init(&stringGoneSemaphore, 0, 1)) < 0)
    fprintf(stderr, "string-taken-out semaphore init failed: %s\n", strerror(rc)), exit(1);
  if ((rc = sem_init(&stringIsInSemaphore, 0, 0)) < 0)
    fprintf(stderr, "string-in-buffer semaphore init failed: %s\n", strerror(rc)), exit(1);
...
/* wait for other thread to terminate then free resources */
  ...
  (void) sem_destroy(&stringGoneSemaphore);
  (void) sem_destroy(&stringIsInSemaphore);
  free(buffer);
